...
sap.ui.define([
	"wiwcloud/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"wiwcloud/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function(BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("wiwcloud.controller.Detail", {
		...
		onDelete: function() {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/VOORNAAM") + " " + this._oODataModel.getProperty(sPath + "/FAMNAAM"),
		...
		onDeleteMail: function() {
			...
			if (aContexts.length === 0) {
				MessageToast.show(this._oResourceBundle.getText("selectEmail"));
			} else {
				var sPath = [];
				for (var i = aContexts.length - 1; i >= 0; i--) {
					sPath[sPath.length] = aContexts[i].sPath;
				}
				var oViewModel = this.getModel("detailView"),
					sQuestion = this._oResourceBundle.getText("emailDeleteText"),
					sSuccessMessage = this._oResourceBundle.getText("emailDeleteSuccess");
		...
		onAdd: function() {
			...
			this.getRouter().getTargets().display("createMail");
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				var sObjectPath = this.getModel().createKey("Person", oParameter);
		...
		_onBindingChange: function() {
			...
			var sPath = oElementBinding.getBoundContext().getPath(),
				...
				sObjectId = oObject.HOOFDPERNR,
				sObjectName = oObject.FAMNAAM;
		...
		_confirmDeletionByUser: function(oConfirmation, aPaths, fnAfterDeleted, fnDeleteCanceled, fnDeleteConfirmed) {
			...
			var fnDelete = function() {
				...
				for (var i = 0; i < aPaths.length; i++) {
					this._callDelete(aPaths[i], fnAfterDeleted);
				}
			}.bind(this);
		...
		_callDelete: function(aPaths, fnAfterDeleted) {
			...
			return this._deleteOneEntity(aPaths, fnSuccess, fnFailed);
		},
